...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    # Initializes empty 2d array the same size as the input image
    filtered_image = np.zeros(image.shape)
    
    # Makes padded array
    padded_image = np.pad(image, ((k // 2, k // 2), # Pad by Filter Height
                                  (j // 2, j // 2), # Pad by Filter Width
                                  (0, 0)), mode='constant')
    
    # Applies filter over each pixel
    for channel in range(c):
      for x in range(m):
        for y in range(n):
          region = padded_image[x:x+k, y:y+j, channel]
