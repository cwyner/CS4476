...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        kernel_2d = create_Gaussian_kernel_2D(cutoff_frequency) # (k, k)
        kernel_3d = np.tile(kernel_2d, (self.n_channels, 1, 1)) # (c, k, k)
        kernel = torch.Tensor(kernel_3d).unsqueeze(1) # (c, 1, k, k)
